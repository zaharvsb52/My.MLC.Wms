...
{
    [TestFixture]
    internal class OrderReserveTest
    {
        private const string QueueMessageTypeCode = "ORDER_RESERVE";
        private const string MandantCode = "TST";
        private const string EventKindCode = "OWB_ACTIVATED";
        private WorkflowIdentity _wfIdentity = new WorkflowIdentity("IntegrationOutOrderReserve", new Version("1.0.0.0"), "INTEGRATION.OUT");
        private IntegrationOutOrderReserve _wf = new IntegrationOutOrderReserve();
        ...
        public void Test()
        {
            _testName = "OrderReserveTest_" + DateTime.UtcNow.ToString("yyMMddhhmmss").Trim();
            ...
            using (var childContainer = _container.CreateChildContainer())
            {
                ...
                using (var transaction = session.BeginTransaction())
                {
                    ...
                    var qout = session.Query<IoQueueOut>()
                        .Where(i => i.Uri == string.Format("ENTITYKEY={0}; ENTITY=OWB", iwb.IWBName) && i.QueueMessageType.Code == QueueMessageTypeCode)
